...
export async function getMembershipAvailability({ cell, designationCode, level, hrcCountryId, zone, hrcStateId, hrcDistrictId, hrcMandalId, includeAggregate = true }){
  ...
  if(!res.ok) {
    ...
    try { const errJson = await res.json(); errMsg = errJson.message || errJson.error || errMsg } catch {}
...
export async function getPublicMembers({ level, hrcCountryId, zone, hrcStateId, hrcDistrictId, hrcMandalId, q, page = 1, pageSize = 24 } = {}){
...
export function getCardWorkPlace(card = {}) {
  const fromLevelLocation = [
    card.levelLocation?.stateName,
    card.levelLocation?.districtName,
    card.levelLocation?.mandalName,
  ].filter(Boolean).join(', ')
  const fromRootLocation = [
    card.stateName,
    card.districtName,
    card.mandalName,
  ].filter(Boolean).join(', ')
...
export function getCardQrUrl(cardNumber) {
  if (!cardNumber) return ''
  return `/api/idcard/${encodeURIComponent(cardNumber)}/qr`
}

// HRCI ID Card by card number
export async function getHRCIIdCard(cardNumber){
  if (!cardNumber) throw new Error('cardNumber is required')
  const url = `${BASE}/hrci/idcard/${encodeURIComponent(cardNumber)}`
  const res = await fetchWithTimeout(url, {}, 8000)
  if (!res.ok) throw new Error('Failed to fetch ID card')
  const json = await res.json()
  return json.data
}
