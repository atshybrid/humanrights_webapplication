export default function TrustBar() {
  ...
  return (
    <section aria-label="Trust and safety" className="border-y border-gray-200 bg-white/70 backdrop-blur">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 px-4 py-4 sm:grid-cols-4 sm:gap-4 sm:px-6">
        {items.map((it) => (
          <div key={it.title} className="flex items-start gap-2 text-sm sm:items-center">
            <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary to-secondary text-white shadow-sm ring-1 ring-white/30">
              {it.icon}
            </span>
            <div className="min-w-0 leading-tight">
              <div className="font-semibold text-gray-900">{it.title}</div>
              <div className="text-xs text-gray-600 sm:text-sm">{it.desc}</div>
            </div>
          </div>
        ))}
      </div>
